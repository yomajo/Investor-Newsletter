...
class LrtScrapper(Scrapper):
    ...
    LRT does not have feature articles, therefore no corresponding call to get_feature_article within category'''
    ...
    def get_category_articles(self, content_container:object, category:str):
        ...
        try:    
            all_category_divs = content_container.findAll('div', class_='news')
            for article_div in all_category_divs:
                # article url
                article_url_unval = article_div.h3.a['href']
                ...
                if article_url not in self.unique_urls_set:
                    ...
                    # headline
                    article_headline = article_div.h3.a.text.strip()    
                    
                    # article img url
                    try:
                        img_url_raw = article_div.img['data-src']
                        article_img_url = self.validate_url(img_url_raw)
                        img_path = download_img(article_img_url, self.user_agent)
                    except TypeError:
    ...
    def scrape_category(self, response:object, category:str):
        ...
        if response != None:
            content_container = self.get_content_container(response, 'div', 'news-list')
